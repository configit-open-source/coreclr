...
namespace System
{
    public struct Guid : IFormattable, IComparable, IComparable<Guid>, IEquatable<Guid>
    {
        ...
        private static bool TryParseGuidWithHexPrefix(String guidString, ref GuidResult result)
        {
            ...
            if (!StringToInt(guidString.Substring(numStart, numLen), -1, ParseNumbers.IsTight, out result.parsedGuid._a, ref result))
                return false;
            ...
            if (!StringToShort(guidString.Substring(numStart, numLen), -1, ParseNumbers.IsTight, out result.parsedGuid._b, ref result))
                return false;
            if (!IsHexPrefix(guidString, numStart + numLen + 1))
            {
            ...
            if (!StringToShort(guidString.Substring(numStart, numLen), -1, ParseNumbers.IsTight, out result.parsedGuid._c, ref result))
                return false;
        ...
        private static bool TryParseGuidWithNoStyle(String guidString, ref GuidResult result)
        {
            ...
            if (!StringToInt(guidString.Substring(startPos, 8), -1, ParseNumbers.IsTight, out result.parsedGuid._a, ref result))
                return false;
            startPos += 8;
        ...
        private static bool TryParseGuidWithDashes(String guidString, ref GuidResult result)
        {
            ...
            if (guidString[8 + startPos] != '-' || guidString[13 + startPos] != '-' || guidString[18 + startPos] != '-' || guidString[23 + startPos] != '-')
            {
