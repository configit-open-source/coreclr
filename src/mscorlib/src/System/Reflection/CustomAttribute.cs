...
namespace System.Reflection
{
    public class CustomAttributeData
    {
        ...
        internal unsafe static CustomAttributeRecord[] GetCustomAttributeRecords(RuntimeModule module, int targetToken)
        {
            ...
            for (int i = 0; i < records.Length; i++)
            {
                scope.GetCustomAttributeProps(tkCustomAttributeTokens[i], out records[i].tkCtor.Value, out records[i].blob);
            }
        ...
        private CustomAttributeData(RuntimeModule scope, CustomAttributeRecord caRecord)
        {
            ...
            for (int i = 0; i < fields.Length; i++)
                m_namedParams[i] = new CustomAttributeNamedParameter(fields[i].Name, CustomAttributeEncoding.Field, InitCustomAttributeType((RuntimeType)fields[i].FieldType));
            for (int i = 0; i < properties.Length; i++)
                m_namedParams[i + fields.Length] = new CustomAttributeNamedParameter(properties[i].Name, CustomAttributeEncoding.Property, InitCustomAttributeType((RuntimeType)properties[i].PropertyType));
        ...
        private void Init(DllImportAttribute dllImport)
        {
            ...
            m_typedCtorArgs = Array.AsReadOnly(new CustomAttributeTypedArgument[]{new CustomAttributeTypedArgument(dllImport.Value), });
        ...
        private void Init(FieldOffsetAttribute fieldOffset)
        {
            ...
            m_typedCtorArgs = Array.AsReadOnly(new CustomAttributeTypedArgument[]{new CustomAttributeTypedArgument(fieldOffset.Value)});
        ...
        private void Init(MarshalAsAttribute marshalAs)
        {
            ...
            m_ctor = type.GetConstructors(BindingFlags.Public | BindingFlags.Instance)[0];
            m_typedCtorArgs = Array.AsReadOnly(new CustomAttributeTypedArgument[]{new CustomAttributeTypedArgument(marshalAs.Value), });
            int i = 3;
        ...
        public virtual IList<CustomAttributeNamedArgument> NamedArguments
        {
            get
            {
                if (m_namedArgs == null)
                {
                    ...
                    {
                        if (m_namedParams[i].EncodedArgument.CustomAttributeType.EncodedType != CustomAttributeEncoding.Undefined)
                            namedArgs[j++] = new CustomAttributeNamedArgument(m_members[i], new CustomAttributeTypedArgument(m_scope, m_namedParams[i].EncodedArgument));
                    }
    ...
    public struct CustomAttributeNamedArgument
    {
        ...
        internal Type ArgumentType
        {
            get
            {
                return m_memberInfo is FieldInfo ? ((FieldInfo)m_memberInfo).FieldType : ((PropertyInfo)m_memberInfo).PropertyType;
            }
        }
    ...
    internal struct CustomAttributeEncodedArgument
    {
        ...
        internal static void ParseAttributeArguments(ConstArray attributeBlob, ref CustomAttributeCtorParameter[] customAttributeCtorParameters, ref CustomAttributeNamedParameter[] customAttributeNamedParameters, RuntimeModule customAttributeModule)
        {
            ...
            if (customAttributeCtorParameters.Length != 0 || customAttributeNamedParameters.Length != 0)
            {
                unsafe
                {
                    ParseAttributeArguments(attributeBlob.Signature, (int)attributeBlob.Length, ref customAttributeCtorParameters, ref customAttributeNamedParameters, (RuntimeAssembly)customAttributeModule.Assembly);
                }
            }
        }
    ...
    internal unsafe static class CustomAttribute
    {
        ...
        internal static Object[] GetCustomAttributes(RuntimeType type, RuntimeType caType, bool inherit)
        {
            ...
            if (!inherit || (caType.IsSealed && !CustomAttribute.GetAttributeUsage(caType).Inherited))
            {
                ...
                if (pcaCount > 0)
                    Array.Copy(pca, 0, attributes, attributes.Length - pcaCount, pcaCount);
                return attributes;
            }
        ...
        private static bool AllowCriticalCustomAttributes(MethodBase method)
        {
            ...
            return !method.IsSecurityTransparent || SpecialAllowCriticalAttributes((RuntimeType)method.DeclaringType);
        }

        private static bool AllowCriticalCustomAttributes(RuntimeFieldInfo field)
        {
            return !field.IsSecurityTransparent || SpecialAllowCriticalAttributes((RuntimeType)field.DeclaringType);
        }
        ...
        private static bool IsCustomAttributeDefined(RuntimeModule decoratedModule, int decoratedMetadataToken, RuntimeType attributeFilterType, int attributeCtorToken, bool mustBeInheritable)
        {
            ...
            if (attributeFilterType != null)
            {
                ...
                for (int i = 0; i < car.Length; i++)
                {
                    ...
                    if (FilterCustomAttributeRecord(caRecord, scope, ref lastAptcaOkAssembly, decoratedModule, decoratedMetadataToken, attributeFilterType, mustBeInheritable, null, null, out attributeType, out ctor, out ctorHasParameters, out isVarArg))
                        return true;
                }
            }
            else
        ...
        private unsafe static object[] GetCustomAttributes(RuntimeModule decoratedModule, int decoratedMetadataToken, int pcaCount, RuntimeType attributeFilterType, bool isDecoratedTargetSecurityTransparent)
        {
        ...
        private unsafe static object[] GetCustomAttributes(RuntimeModule decoratedModule, int decoratedMetadataToken, int pcaCount, RuntimeType attributeFilterType, bool mustBeInheritable, IList derivedAttributes, bool isDecoratedTargetSecurityTransparent)
        {
            ...
            for (int i = 0; i < car.Length; i++)
            {
                ...
                if (!FilterCustomAttributeRecord(caRecord, scope, ref lastAptcaOkAssembly, decoratedModule, decoratedMetadataToken, attributeFilterType, mustBeInheritable, attributes, derivedAttributes, out attributeType, out ctor, out ctorHasParameters, out isVarArg))
                    continue;
                if (ctor != null)
                {
                ...
                else
                {
                    ...
                    else
                    {
                        ...
                        blobStart = (IntPtr)((byte *)blobStart + 2);
                ...
                for (int j = 0; j < cNamedArgs; j++)
                {
                    ...
                    {
                        throw new CustomAttributeFormatException(String.Format(CultureInfo.CurrentUICulture, Environment.GetResourceString(isProperty ? "RFLCT.InvalidPropFail" : "RFLCT.InvalidFieldFail"), name), e);
                    }
                }
        ...
        private unsafe static bool FilterCustomAttributeRecord(CustomAttributeRecord caRecord, MetadataImport scope, ref Assembly lastAptcaOkAssembly, RuntimeModule decoratedModule, MetadataToken decoratedToken, RuntimeType attributeFilterType, bool mustBeInheritable, object[] attributes, IList derivedAttributes, out RuntimeType attributeType, out IRuntimeMethodInfo ctor, out bool ctorHasParameters, out bool isVarArg)
        {
            ...
            {
                Contract.Assert(decoratedToken.IsModule || decoratedToken.IsAssembly, "The decoratedToken must be either an assembly, a module, a type, or a member.");
            }

            RuntimeTypeHandle parentTypeHandle = tkParent.IsTypeDef ? decoratedModule.ModuleHandle.ResolveTypeHandle(tkParent) : new RuntimeTypeHandle();
        ...
        internal static AttributeUsageAttribute GetAttributeUsage(RuntimeType decoratedAttribute)
        {
            ...
            for (int i = 0; i < car.Length; i++)
            {
                ...
                if (attributeType != (RuntimeType)typeof (AttributeUsageAttribute))
                    continue;
                if (attributeUsageAttribute != null)
                    throw new FormatException(String.Format(CultureInfo.CurrentUICulture, Environment.GetResourceString("Format_AttributeUsage"), attributeType));
        ...
        private unsafe extern static void _GetPropertyOrFieldData(RuntimeModule pModule, byte **ppBlobStart, byte *pBlobEnd, out string name, out bool bIsProperty, out RuntimeType type, out object value);
        private unsafe static void GetPropertyOrFieldData(RuntimeModule module, ref IntPtr blobStart, IntPtr blobEnd, out string name, out bool isProperty, out RuntimeType type, out object value)
        {
            ...
            _GetPropertyOrFieldData(module.GetNativeHandle(), &pBlobStart, (byte *)blobEnd, out name, out isProperty, out type, out value);
    ...
    internal static class PseudoCustomAttribute
    {
        ...
        {
            RuntimeType[] pcas = new RuntimeType[]{typeof (FieldOffsetAttribute) as RuntimeType, typeof (MarshalAsAttribute) as RuntimeType, typeof (ComImportAttribute) as RuntimeType, typeof (InAttribute) as RuntimeType, typeof (OutAttribute) as RuntimeType, typeof (OptionalAttribute) as RuntimeType, typeof (DllImportAttribute) as RuntimeType, typeof (PreserveSigAttribute) as RuntimeType, typeof (TypeForwardedToAttribute) as RuntimeType, };
        ...
        internal static bool IsDefined(RuntimeType type, RuntimeType caType)
        {
            ...
            if (all || caType == (RuntimeType)typeof (ComImportAttribute))
            {
                if (ComImportAttribute.IsDefined(type))
                    return true;
            }
        ...
        internal static bool IsDefined(RuntimeMethodInfo method, RuntimeType caType)
        {
            ...
            if (all || caType == (RuntimeType)typeof (DllImportAttribute))
            {
                if (DllImportAttribute.IsDefined(method))
                    return true;
            }

            if (all || caType == (RuntimeType)typeof (PreserveSigAttribute))
            {
                if (PreserveSigAttribute.IsDefined(method))
                    return true;
            }
        ...
        internal static bool IsDefined(RuntimeParameterInfo parameter, RuntimeType caType)
        {
            ...
            if (all || caType == (RuntimeType)typeof (InAttribute))
            {
                if (InAttribute.IsDefined(parameter))
                    return true;
            }

            if (all || caType == (RuntimeType)typeof (OutAttribute))
            {
                if (OutAttribute.IsDefined(parameter))
                    return true;
            }

            if (all || caType == (RuntimeType)typeof (OptionalAttribute))
            {
                if (OptionalAttribute.IsDefined(parameter))
                    return true;
            }

            if (all || caType == (RuntimeType)typeof (MarshalAsAttribute))
            {
                if (MarshalAsAttribute.IsDefined(parameter))
                    return true;
            }
        ...
        internal static bool IsDefined(RuntimeFieldInfo field, RuntimeType caType)
        {
            ...
            if (all || caType == (RuntimeType)typeof (MarshalAsAttribute))
            {
                if (MarshalAsAttribute.IsDefined(field))
                    return true;
            }

            if (all || caType == (RuntimeType)typeof (FieldOffsetAttribute))
            {
                if (FieldOffsetAttribute.IsDefined(field))
                    return true;
            }
