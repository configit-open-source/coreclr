    ...
    public sealed class RijndaelManagedTransform : ICryptoTransform
    {
            ...
            else
            {
                ...
                else
                {
                    if (m_depadBuffer == null)
                    {
                        ...
                        return DecryptData(inputBuffer, inputOffset, inputToProcess, ref outputBuffer, outputOffset, m_paddingValue, false);
                    }
                    else
                    {
                        int r = DecryptData(m_depadBuffer, 0, m_depadBuffer.Length, ref outputBuffer, outputOffset, m_paddingValue, false);
                        outputOffset += OutputBlockSize;
                        int inputToProcess = inputCount - InputBlockSize;
                        Buffer.InternalBlockCopy(inputBuffer, inputOffset + inputToProcess, m_depadBuffer, 0, InputBlockSize);
                        r = DecryptData(inputBuffer, inputOffset, inputToProcess, ref outputBuffer, outputOffset, m_paddingValue, false);
        ...
        public byte[] TransformFinalBlock(byte[] inputBuffer, int inputOffset, int inputCount)
        {
            ...
            else
            {
                ...
                {
                    byte[] transformedBytes = null;
                    DecryptData(inputBuffer, inputOffset, inputCount, ref transformedBytes, 0, m_paddingValue, true);
                    Reset();
                    return transformedBytes;
                }
                else
                {
                    ...
                    byte[] transformedBytes = null;
                    DecryptData(temp, 0, temp.Length, ref transformedBytes, 0, m_paddingValue, true);
                    Reset();
                    return transformedBytes;
                }
            }
                            ...
                            for (int blockNum = 0; blockNum < iNumBlocks; ++blockNum)
                            {
                                ...
                                if (m_cipherMode == CipherMode.CFB)
                                {
                                    ...
                                    if (blockNum != iNumBlocks - 1 || padSize == 0)
                                    {
                                        for (int i = 0; i < m_Nb; ++i)
                                        {
                                            if (index >= workBaseIndex + m_inputBlockSize)
                                                break;
        ...
        private unsafe void Enc(int *encryptindex, int *encryptKeyExpansion, int *T, int *TF, int *work, int *temp)
        {
            ...
            for (int r = 1; r < m_Nr; ++r)
            {
                ...
                for (int i = 0; i < m_Nb; ++i)
                {
                    temp[i] = T[0 + (work[i] & 0xFF)] ^ T[256 + ((work[_encryptindex[0]] >> 8) & 0xFF)] ^ T[512 + ((work[_encryptindex[m_Nb]] >> 16) & 0xFF)] ^ T[768 + ((work[_encryptindex[m_Nb * 2]] >> 24) & 0xFF)] ^ *_encryptKeyExpansion;
                    _encryptindex++;
            ...
            for (int i = 0; i < m_Nb; ++i)
            {
                temp[i] = TF[0 + (work[i] & 0xFF)] ^ TF[256 + ((work[_encryptindex[0]] >> 8) & 0xFF)] ^ TF[512 + ((work[_encryptindex[m_Nb]] >> 16) & 0xFF)] ^ TF[768 + ((work[_encryptindex[m_Nb * 2]] >> 24) & 0xFF)] ^ *_encryptKeyExpansion;
                _encryptindex++;
                _encryptKeyExpansion++;
            }
        }
        ...
        unsafe private void Dec(int *decryptindex, int *decryptKeyExpansion, int *iT, int *iTF, int *work, int *temp)
        {
            ...
            for (int r = 1; r < m_Nr; ++r)
            {
                ...
                for (int i = 0; i < m_Nb; ++i)
                {
                    temp[i] = iT[0 + ((work[i]) & 0xFF)] ^ iT[256 + ((work[_decryptindex[0]] >> 8) & 0xFF)] ^ iT[512 + ((work[_decryptindex[m_Nb]] >> 16) & 0xFF)] ^ iT[768 + ((work[_decryptindex[m_Nb * 2]] >> 24) & 0xFF)] ^ *_decryptKeyExpansion;
                    keyIndex++;
                    _decryptindex++;
            ...
            for (int i = 0; i < m_Nb; ++i)
            {
                temp[i] = iTF[0 + ((work[i]) & 0xFF)] ^ iTF[256 + ((work[_decryptindex[0]] >> 8) & 0xFF)] ^ iTF[512 + ((work[_decryptindex[m_Nb]] >> 16) & 0xFF)] ^ iTF[768 + ((work[_decryptindex[m_Nb * 2]] >> 24) & 0xFF)] ^ *_decryptKeyExpansion;
                _decryptindex++;
                _decryptKeyExpansion++;
            }
        }
